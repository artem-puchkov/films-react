...
export function FilmsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      const newPartOfFilms: FilmCardData[] = await getPartOfRatingFilms(
        currentPage
      );
  ...
  useEffect(() => {
    async function fetchData() {
      const newPartOfFilms: FilmCardData[] = await getPartOfPopularFilms(
        currentPage
      );
      setFilms(newPartOfFilms);
    }
...
async function getPartOfRatingFilms(page: number) {
  ...
  return page === 1 ? films.slice(0, 6) : films.slice(6 * (page - 1), 6 * page);
}

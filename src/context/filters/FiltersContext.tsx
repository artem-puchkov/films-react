...
import { GenresFromApi, Genres, GenreAction, RatingSortAction } from "../../components/Filters/Filter.type";
...
const GenresDispatchContext = createContext<React.Dispatch<GenreAction> | null>(null);

const RatingSortContext = createContext<string | null>(null)
const RatingSortDispatchContext = createContext<React.Dispatch<RatingSortAction> | null>(null)

export function FiltersProvider({ children }: ProviderProps) {
    const [genres, dispatchGenres] = useReducer(genresReducer, initialGenresObject);
    const [ratingSort, dispatchRatingSort] = useReducer(ratingSortReducer, "nothing");
...
const initialGenresObject: Genres = initialGenres.reduce((acc: Genres, genre) => {
    acc[genre] = {
